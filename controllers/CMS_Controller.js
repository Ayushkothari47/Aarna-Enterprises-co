...
// Upload banners controller
exports.uploadMobileBanners = async (req, res) => {
  try {
    const files = req.files;
    ...
    for (const file of files) {
      const result = await cloudinary.uploader.upload(file.path, { folder: "MobileBanners" });
      uploadedBannerUrls.push(result.secure_url);
    }
...
// GET /api/banners - Fetch all banner URLs     - FOR ADMINS
exports.getAllMobileBanners = async (req, res) => {
  try {
    // Fetch all banners from DB
...
exports.getMobileBanners = async (req, res) => {
  try {
    // Fetch only banners where isVisible is true
    const banners = await MobileBanner.find({ isVisible: true });
...
exports.deleteMobileBanner = async (req, res) => {
  try {
    ...
    // Find and update document by pulling out banner URL
    const updated = await MobileBanner.findOneAndUpdate(
      {},
      { $pull: { banners: bannerUrl } },
      { new: true }
    );
...
// PUT /CMS/updateBannerVisibility
exports.updateMobileBannerVisibility = async (req, res) => {
  try {
    const { isVisible } = req.body;
...
// POST /CMS/addBanner
exports.addMobileBanner = async (req, res) => {
  try {
    const file = req.file; // single file (use multer.single("banner"))
